...
extern {
     ...
     */
    #[no_mangle]
    fn zbar_decoder_get_data(decoder: *const zbar_decoder_t) -> *const libc::c_char;
    /* * retrieve length of binary data.
     * @returns the length of the decoded data or 0 if no new data
     * available.
     */
    #[no_mangle]
    fn zbar_decoder_get_data_length(decoder: *const zbar_decoder_t) -> libc::c_uint;
    /* * retrieve last decoded symbol type.
     * @returns the type or ::ZBAR_NONE if no new data available
     */
    #[no_mangle]
    fn zbar_decoder_get_type(decoder: *const zbar_decoder_t) -> zbar_symbol_type_t;
     ...
     */
    #[no_mangle]
    fn zbar_decoder_get_modifiers(decoder: *const zbar_decoder_t) -> libc::c_uint;
     ...
     */
    #[no_mangle]
    fn zbar_decoder_get_direction(decoder: *const zbar_decoder_t) -> libc::c_int;
     ...
     */
    #[no_mangle]
    fn zbar_decoder_set_handler(
        decoder: *mut zbar_decoder_t,
        handler: Option<zbar_decoder_handler_t>,
    ) -> Option<zbar_decoder_handler_t>;
    /* * associate user specified data value with the decoder. */
    #[no_mangle]
    fn zbar_decoder_set_userdata(decoder: *mut zbar_decoder_t, userdata: *mut libc::c_void);
    /* * return user specified data value associated with the decoder. */
    #[no_mangle]
    fn zbar_decoder_get_userdata(decoder: *const zbar_decoder_t) -> *mut libc::c_void;
    /* * constructor.
    ...
    #[no_mangle]
    fn _zbar_decoder_get_qr_finder_line(_: *mut zbar_decoder_t) -> *mut qr_finder_line;
}
...
#[inline]
unsafe extern fn _zbar_timer_now() -> libc::c_int {
    ...
    return (now.tv_sec * 1000 as libc::c_int as libc::c_long
        + now.tv_nsec / 1000000 as libc::c_int as libc::c_long) as libc::c_int;
}
#[no_mangle]
pub unsafe extern fn _zbar_image_scanner_recycle_syms(
    ...
    while !sym.is_null() {
        next = (*sym).next;
        if (*sym).refcnt != 0 && _zbar_refcnt(&mut (*sym).refcnt, -(1 as libc::c_int)) != 0 {
            ...
            let mut bucket: *mut recycle_bucket_t = 0 as *mut recycle_bucket_t;
            ...
            if !(*sym).syms.is_null() {
                if _zbar_refcnt(&mut (*(*sym).syms).refcnt, -(1 as libc::c_int)) != 0 {
            ...
            while i < 5 as libc::c_int {
                if (*sym).data_alloc < ((1 as libc::c_int) << i * 2 as libc::c_int) as libc::c_uint
                {
                    break;
                }
                i += 1
            ...
            bucket = &mut *(*iscn).recycle.as_mut_ptr().offset(i as isize) as *mut recycle_bucket_t;
...
#[inline]
#[linkage = "external"]
pub unsafe extern fn zbar_image_scanner_recycle_image(
    mut iscn: *mut zbar_image_scanner_t,
...
#[inline]
#[linkage = "external"]
pub unsafe extern fn _zbar_image_scanner_alloc_sym(
    mut iscn: *mut zbar_image_scanner_t,
    mut type_0: zbar_symbol_type_t,
    ...
    while i < 5 as libc::c_int - 1 as libc::c_int {
        if datalen <= (1 as libc::c_int) << i * 2 as libc::c_int {
            break;
        }
        i += 1
    }
    ...
    if !sym.is_null() {
        ...
        sym = calloc(
            ...
            ::std::mem::size_of::<zbar_symbol_t>() as libc::c_ulong,
        ) as *mut zbar_symbol_t;
        (*iscn).stat_sym_new += 1
    }
    /* init new symbol */
    (*sym).type_0 = type_0;
    ...
    if datalen > 0 as libc::c_int {
        ...
        if (*sym).data_alloc < datalen as libc::c_uint {
            ...
            (*sym).data = malloc(datalen as libc::c_ulong) as *mut libc::c_char
        }
    } else {
        if !(*sym).data.is_null() {
            free((*sym).data as *mut libc::c_void);
...
#[inline]
unsafe extern fn cache_sym(mut iscn: *mut zbar_image_scanner_t, mut sym: *mut zbar_symbol_t) {
    if (*iscn).enable_cache != 0 {
        ...
        if entry.is_null() {
            ...
            entry = _zbar_image_scanner_alloc_sym(
                iscn,
                (*sym).type_0,
                (*sym).datalen.wrapping_add(1 as libc::c_int as libc::c_uint) as libc::c_int,
            );
            ...
            (*entry).time = (*sym).time.wrapping_sub(2000 as libc::c_int as libc::c_ulong);
        ...
        (*entry).time = (*sym).time;
        near_thresh = (age < 1000 as libc::c_int as libc::c_uint) as libc::c_int as uint32_t;
        far_thresh = (age >= 2000 as libc::c_int as libc::c_uint) as libc::c_int as uint32_t;
        dup = ((*entry).cache_count >= 0 as libc::c_int) as libc::c_int as uint32_t;
        if dup == 0 && near_thresh == 0 || far_thresh != 0 {
            let mut type_0: libc::c_int = (*sym).type_0 as libc::c_int;
            let mut h: libc::c_int = _zbar_get_symbol_hash(type_0 as zbar_symbol_type_t);
            (*entry).cache_count = -(*iscn).sym_configs[0 as libc::c_int as usize][h as usize]
        } else if dup != 0 || near_thresh != 0 {
            (*entry).cache_count += 1
        }
...
#[no_mangle]
pub unsafe extern fn _zbar_image_scanner_add_sym(
    mut iscn: *mut zbar_image_scanner_t,
    mut sym: *mut zbar_symbol_t,
) {
...
#[inline]
unsafe extern fn qr_handler(mut iscn: *mut zbar_image_scanner_t) {
    let mut u: libc::c_uint = 0;
    let mut vert: libc::c_int = 0;
    let mut line: *mut qr_finder_line = _zbar_decoder_get_qr_finder_line((*iscn).dcode);
    if !line.is_null() {
    } else {
        __assert_fail(
            b"line\x00" as *const u8 as *const libc::c_char,
            b"zbar/img_scanner.c\x00" as *const u8 as *const libc::c_char,
            367 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 40], &[libc::c_char; 40]>(
                b"void qr_handler(zbar_image_scanner_t *)\x00",
            ))
            .as_ptr(),
        );
    }
    u = zbar_scanner_get_edge(
        (*iscn).scn,
        (*line).pos[0 as libc::c_int as usize] as libc::c_uint,
        2 as libc::c_int,
    );
    (*line).boffs = u.wrapping_sub(zbar_scanner_get_edge(
        (*iscn).scn,
        (*line).boffs as libc::c_uint,
        2 as libc::c_int,
    )) as libc::c_int;
    (*line).len = zbar_scanner_get_edge((*iscn).scn, (*line).len as libc::c_uint, 2 as libc::c_int)
        as libc::c_int;
    (*line).eoffs =
        zbar_scanner_get_edge((*iscn).scn, (*line).eoffs as libc::c_uint, 2 as libc::c_int)
            .wrapping_sub((*line).len as libc::c_uint) as libc::c_int;
    (*line).len = ((*line).len as libc::c_uint).wrapping_sub(u) as libc::c_int as libc::c_int;
    u = (((((*iscn).umin << 1 as libc::c_int) + 0 as libc::c_int)
        << 2 as libc::c_int - 1 as libc::c_int) as libc::c_uint)
        .wrapping_add(((*iscn).du as libc::c_uint).wrapping_mul(u));
    ...
    (*line).pos[(vert == 0) as libc::c_int as usize] =
        (((*iscn).v << 1 as libc::c_int) + 1 as libc::c_int) << 2 as libc::c_int - 1 as libc::c_int;
    _zbar_qr_found_line((*iscn).qr, vert, line);
}
unsafe extern fn symbol_handler(mut dcode: *mut zbar_decoder_t) {
    ...
    if (*iscn).config >> ZBAR_CFG_POSITION as libc::c_int - ZBAR_CFG_POSITION as libc::c_int
        & 1 as libc::c_int as libc::c_uint
        != 0
    {
        /* tmp position fixup */
        let mut w: libc::c_int = zbar_scanner_get_width((*iscn).scn) as libc::c_int;
        let mut u: libc::c_int =
            ((*iscn).umin as libc::c_uint).wrapping_add(((*iscn).du as libc::c_uint).wrapping_mul(
                zbar_scanner_get_edge((*iscn).scn, w as libc::c_uint, 0 as libc::c_int),
            )) as libc::c_int;
        if (*iscn).dx != 0 {
            x = u;
            y = (*iscn).v
    ...
    if type_0 as libc::c_uint <= ZBAR_PARTIAL as libc::c_int as libc::c_uint {
        if _zbar_verbosity >= 256 as libc::c_int {
            fprintf(
                ...
                (*::std::mem::transmute::<&[u8; 15], &[libc::c_char; 15]>(b"symbol_handler\x00"))
                    .as_ptr(),
                x,
                y,
            );
    ...
    sym = _zbar_image_scanner_alloc_sym(
        iscn,
        type_0,
        datalen.wrapping_add(1 as libc::c_int as libc::c_uint) as libc::c_int,
    );
    (*sym).configs = zbar_decoder_get_configs(dcode, type_0);
    (*sym).modifiers = zbar_decoder_get_modifiers(dcode);
    /* FIXME grab decoder buffer */
    memcpy(
        (*sym).data as *mut libc::c_void,
        data as *const libc::c_void,
        datalen.wrapping_add(1 as libc::c_int as libc::c_uint) as libc::c_ulong,
    );
    ...
    if dir != 0 {
        (*sym).orient = (((*iscn).dy != 0 as libc::c_int) as libc::c_int
            + (((*iscn).du ^ dir) & 2 as libc::c_int)) as zbar_orientation_t
    }
    _zbar_image_scanner_add_sym(iscn, sym);
}
/* * constructor. */
#[no_mangle]
pub unsafe extern fn zbar_image_scanner_create() -> *mut zbar_image_scanner_t {
    let mut iscn: *mut zbar_image_scanner_t = calloc(
        1 as libc::c_int as libc::c_ulong,
    ...
    zbar_decoder_set_handler(
        (*iscn).dcode,
        Some(symbol_handler as unsafe extern fn(_: *mut zbar_decoder_t) -> ()),
    );
    (*iscn).qr = _zbar_qr_create();
    /* apply default configuration */
    (*iscn).configs
        [(ZBAR_CFG_X_DENSITY as libc::c_int - ZBAR_CFG_X_DENSITY as libc::c_int) as usize] =
        1 as libc::c_int;
    (*iscn).configs
        [(ZBAR_CFG_Y_DENSITY as libc::c_int - ZBAR_CFG_X_DENSITY as libc::c_int) as usize] =
        1 as libc::c_int;
    zbar_image_scanner_set_config(iscn, ZBAR_NONE, ZBAR_CFG_POSITION, 1 as libc::c_int);
    zbar_image_scanner_set_config(iscn, ZBAR_NONE, ZBAR_CFG_UNCERTAINTY, 2 as libc::c_int);
    zbar_image_scanner_set_config(iscn, ZBAR_QRCODE, ZBAR_CFG_UNCERTAINTY, 0 as libc::c_int);
    zbar_image_scanner_set_config(iscn, ZBAR_CODE128, ZBAR_CFG_UNCERTAINTY, 0 as libc::c_int);
    zbar_image_scanner_set_config(iscn, ZBAR_CODE93, ZBAR_CFG_UNCERTAINTY, 0 as libc::c_int);
    zbar_image_scanner_set_config(iscn, ZBAR_CODE39, ZBAR_CFG_UNCERTAINTY, 0 as libc::c_int);
    zbar_image_scanner_set_config(iscn, ZBAR_CODABAR, ZBAR_CFG_UNCERTAINTY, 1 as libc::c_int);
    zbar_image_scanner_set_config(iscn, ZBAR_COMPOSITE, ZBAR_CFG_UNCERTAINTY, 0 as libc::c_int);
    return iscn;
}
#[inline]
unsafe extern fn dump_stats(mut iscn: *const zbar_image_scanner_t) {
    let mut i: libc::c_int = 0;
    if _zbar_verbosity >= 1 as libc::c_int {
        fprintf(
            stderr,
            b"%s: symbol sets allocated   = %-4d\n\x00" as *const u8 as *const libc::c_char,
            (*::std::mem::transmute::<&[u8; 11], &[libc::c_char; 11]>(b"dump_stats\x00")).as_ptr(),
            (*iscn).stat_syms_new,
        );
    }
    if _zbar_verbosity >= 1 as libc::c_int {
    ...
    while i < 5 as libc::c_int {
        if _zbar_verbosity >= 1 as libc::c_int {
            fprintf(
                stderr,
                b"%s:      recycled[%d]        = %-4d\n\x00" as *const u8 as *const libc::c_char,
                (*::std::mem::transmute::<&[u8; 11], &[libc::c_char; 11]>(b"dump_stats\x00"))
                    .as_ptr(),
                i,
                (*iscn).stat_sym_recycle[i as usize],
            );
        }
        i += 1
    }
...
/* * destructor. */
#[no_mangle]
pub unsafe extern fn zbar_image_scanner_destroy(mut iscn: *mut zbar_image_scanner_t) {
    let mut i: libc::c_int = 0;
    dump_stats(iscn);
    if !(*iscn).syms.is_null() {
 ...
 */
#[no_mangle]
pub unsafe extern fn zbar_image_scanner_set_data_handler(
    mut iscn: *mut zbar_image_scanner_t,
    mut handler: Option<zbar_image_data_handler_t>,
 ...
 */
#[no_mangle]
pub unsafe extern fn zbar_image_scanner_set_config(
    mut iscn: *mut zbar_image_scanner_t,
    ...
    if (sym as libc::c_uint == 0 as libc::c_int as libc::c_uint
        || sym as libc::c_uint == ZBAR_COMPOSITE as libc::c_int as libc::c_uint)
        && cfg as libc::c_uint == ZBAR_CFG_ENABLE as libc::c_int as libc::c_uint
    {
    ...
    if (cfg as libc::c_uint) < ZBAR_CFG_UNCERTAINTY as libc::c_int as libc::c_uint {
    ...
    if (cfg as libc::c_uint) < ZBAR_CFG_POSITION as libc::c_int as libc::c_uint {
        ...
        c = (cfg as libc::c_uint).wrapping_sub(ZBAR_CFG_UNCERTAINTY as libc::c_int as libc::c_uint)
            as libc::c_int;
    ...
    if cfg as libc::c_uint >= ZBAR_CFG_X_DENSITY as libc::c_int as libc::c_uint
        && cfg as libc::c_uint <= ZBAR_CFG_Y_DENSITY as libc::c_int as libc::c_uint
    {
        (*iscn).configs[(cfg as libc::c_uint)
            .wrapping_sub(ZBAR_CFG_X_DENSITY as libc::c_int as libc::c_uint)
            as usize] = val;
        return 0 as libc::c_int;
    }
    if cfg as libc::c_uint > ZBAR_CFG_POSITION as libc::c_int as libc::c_uint {
    ...
    cfg = ::std::mem::transmute::<libc::c_uint, zbar_config_t>(
        (cfg as libc::c_uint).wrapping_sub(ZBAR_CFG_POSITION as libc::c_int as libc::c_uint),
    ) as zbar_config_t;
    if val == 0 {
        (*iscn).config &= !((1 as libc::c_int) << cfg as libc::c_uint) as libc::c_uint
 ...
 */
#[no_mangle]
pub unsafe extern fn zbar_image_scanner_enable_cache(
    mut iscn: *mut zbar_image_scanner_t,
 ...
 */
#[no_mangle]
pub unsafe extern fn zbar_image_scanner_get_results(
    mut iscn: *const zbar_image_scanner_t,
) -> *const zbar_symbol_set_t {
    return (*iscn).syms;
}
#[inline]
unsafe extern fn quiet_border(mut iscn: *mut zbar_image_scanner_t) {
 ...
 */
#[no_mangle]
pub unsafe extern fn zbar_scan_image(
    mut iscn: *mut zbar_image_scanner_t,
    mut img: *mut zbar_image_t,
) -> libc::c_int {
    let mut syms: *mut zbar_symbol_set_t = 0 as *mut zbar_symbol_set_t;
    ...
    if syms.is_null() {
        ...
        zbar_symbol_set_ref(syms, 2 as libc::c_int);
    }
    (*img).syms = syms;
    w = (*img).width;
    h = (*img).height;
    ...
    if cx1 <= w {
        ...
        __assert_fail(
            b"cx1 <= w\x00" as *const u8 as *const libc::c_char,
            b"zbar/img_scanner.c\x00" as *const u8 as *const libc::c_char,
            683 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 60], &[libc::c_char; 60]>(
                b"int zbar_scan_image(zbar_image_scanner_t *, zbar_image_t *)\x00",
            ))
            .as_ptr(),
        );
    }
    ...
    if cy1 <= h {
        ...
        __assert_fail(
            b"cy1 <= h\x00" as *const u8 as *const libc::c_char,
            b"zbar/img_scanner.c\x00" as *const u8 as *const libc::c_char,
    ...
    zbar_scanner_new_scan(scn);
    density = (*iscn).configs
        [(ZBAR_CFG_Y_DENSITY as libc::c_int - ZBAR_CFG_X_DENSITY as libc::c_int) as usize];
    if density > 0 as libc::c_int {
        let mut p: *const uint8_t = data;
        let mut x: libc::c_int = 0 as libc::c_int;
        let mut y: libc::c_int = 0 as libc::c_int;
        let mut border: libc::c_int = (*img)
            .crop_h
            .wrapping_sub(1 as libc::c_int as libc::c_uint)
            .wrapping_rem(density as libc::c_uint)
            .wrapping_add(1 as libc::c_int as libc::c_uint)
            .wrapping_div(2 as libc::c_int as libc::c_uint)
            as libc::c_int;
        if border as libc::c_uint > (*img).crop_h.wrapping_div(2 as libc::c_int as libc::c_uint) {
            border = (*img).crop_h.wrapping_div(2 as libc::c_int as libc::c_uint) as libc::c_int
        }
        border = (border as libc::c_uint).wrapping_add((*img).crop_y) as libc::c_int as libc::c_int;
        if border as libc::c_uint <= h {
        } else {
            __assert_fail(
                b"border <= h\x00" as *const u8 as *const libc::c_char,
                b"zbar/img_scanner.c\x00" as *const u8 as *const libc::c_char,
        ...
        x = (x as libc::c_uint).wrapping_add((*img).crop_x) as libc::c_int as libc::c_int;
        y += border;
        p = p.offset(
            ((*img).crop_x as libc::c_ulong)
                .wrapping_add((border as uintptr_t).wrapping_mul(w as libc::c_ulong))
                as isize,
        );
        (*iscn).v = y;
        while (y as libc::c_uint) < cy1 {
            ...
            while (x as libc::c_uint) < cx1 {
                ...
                y += 0 as libc::c_int;
                p =
                    p.offset((1 as libc::c_int as libc::c_ulong).wrapping_add(
                        (0 as libc::c_int as uintptr_t).wrapping_mul(w as libc::c_ulong),
                    ) as isize);
                zbar_scan_y(scn, d as libc::c_int);
            }
            if p == data.offset(x as isize).offset((y as libc::c_long * w as intptr_t) as isize) {
            } else {
                __assert_fail(
                    b"p == data + x + y * (intptr_t)w\x00" as *const u8 as *const libc::c_char,
                    b"zbar/img_scanner.c\x00" as *const u8 as *const libc::c_char,
            ...
            p = p.offset(
                (-(1 as libc::c_int) as libc::c_ulong)
                    .wrapping_add((density as uintptr_t).wrapping_mul(w as libc::c_ulong))
                    as isize,
            );
            (*iscn).v = y;
            if y as libc::c_uint >= cy1 {
                break;
            }
            if _zbar_verbosity >= 128 as libc::c_int {
                fprintf(
    ...
    if density > 0 as libc::c_int {
        ...
        let mut border_0: libc::c_int = (*img)
            .crop_w
            .wrapping_sub(1 as libc::c_int as libc::c_uint)
            .wrapping_rem(density as libc::c_uint)
            .wrapping_add(1 as libc::c_int as libc::c_uint)
            .wrapping_div(2 as libc::c_int as libc::c_uint)
            as libc::c_int;
        if border_0 as libc::c_uint > (*img).crop_w.wrapping_div(2 as libc::c_int as libc::c_uint) {
            border_0 = (*img).crop_w.wrapping_div(2 as libc::c_int as libc::c_uint) as libc::c_int
        }
        border_0 =
            (border_0 as libc::c_uint).wrapping_add((*img).crop_x) as libc::c_int as libc::c_int;
        if border_0 as libc::c_uint <= w {
        } else {
            __assert_fail(
                b"border <= w\x00" as *const u8 as *const libc::c_char,
                b"zbar/img_scanner.c\x00" as *const u8 as *const libc::c_char,
        ...
        x_0 += border_0;
        y_0 = (y_0 as libc::c_uint).wrapping_add((*img).crop_y) as libc::c_int as libc::c_int;
        p_0 = p_0.offset(
            (border_0 as libc::c_ulong)
                .wrapping_add(((*img).crop_y as uintptr_t).wrapping_mul(w as libc::c_ulong))
                as isize,
        );
        (*iscn).v = x_0;
        while (x_0 as libc::c_uint) < cx1 {
            ...
            while (y_0 as libc::c_uint) < cy1 {
                ...
                y_0 += 1 as libc::c_int;
                p_0 =
                    p_0.offset((0 as libc::c_int as libc::c_ulong).wrapping_add(
                        (1 as libc::c_int as uintptr_t).wrapping_mul(w as libc::c_ulong),
                    ) as isize);
                zbar_scan_y(scn, d_1 as libc::c_int);
            }
            if p_0
                == data.offset(x_0 as isize).offset((y_0 as libc::c_long * w as intptr_t) as isize)
            {
            } else {
                __assert_fail(
                    b"p == data + x + y * (intptr_t)w\x00" as *const u8 as *const libc::c_char,
            ...
            y_0 += -(1 as libc::c_int);
            p_0 =
                p_0.offset((density as libc::c_ulong).wrapping_add(
                    (-(1 as libc::c_int) as uintptr_t).wrapping_mul(w as libc::c_ulong),
                ) as isize);
            (*iscn).v = x_0;
            if x_0 as libc::c_uint >= cx1 {
                break;
            }
            if _zbar_verbosity >= 128 as libc::c_int {
                fprintf(
            ...
            while y_0 >= cy0 {
                ...
                x_0 += 0 as libc::c_int;
                y_0 += -(1 as libc::c_int);
                p_0 = p_0.offset((0 as libc::c_int as libc::c_ulong).wrapping_add(
                    (-(1 as libc::c_int) as uintptr_t).wrapping_mul(w as libc::c_ulong),
                ) as isize);
            ...
            y_0 += 1 as libc::c_int;
            p_0 = p_0.offset(
                (density as libc::c_ulong)
                    .wrapping_add((1 as libc::c_int as uintptr_t).wrapping_mul(w as libc::c_ulong))
                    as isize,
            );
            (*iscn).v = x_0
        }
    ...
    let mut filter: libc::c_char = ((*iscn).enable_cache == 0
    ...
    if (*syms).nsyms != 0 {
        ...
        while !(*symp).is_null() {
            ...
            if (*sym).cache_count <= 0 as libc::c_int
                && (((*sym).type_0 as libc::c_uint) < ZBAR_COMPOSITE as libc::c_int as libc::c_uint
                    && (*sym).type_0 as libc::c_uint > ZBAR_PARTIAL as libc::c_int as libc::c_uint
                    || (*sym).type_0 as libc::c_uint == ZBAR_DATABAR as libc::c_int as libc::c_uint
                    || (*sym).type_0 as libc::c_uint
                        == ZBAR_DATABAR_EXP as libc::c_int as libc::c_uint
                    || (*sym).type_0 as libc::c_uint == ZBAR_CODABAR as libc::c_int as libc::c_uint)
            {
                if ((*sym).type_0 as libc::c_uint == ZBAR_CODABAR as libc::c_int as libc::c_uint
                    ...
                    && (*sym).quality < 4 as libc::c_int
                {
                    ...
                    *symp = (*sym).next;
                    ...
                    && (*sym).type_0 as libc::c_uint != ZBAR_ISBN10 as libc::c_int as libc::c_uint
                {
                    if (*sym).type_0 as libc::c_uint > ZBAR_EAN5 as libc::c_int as libc::c_uint {
                        nean += 1
                    } else {
                        naddon += 1
                    }
        ...
        if nean == 1 as libc::c_int && naddon == 1 as libc::c_int && (*iscn).ean_config != 0 {
            ...
            while !(*symp).is_null() {
                ...
                if ((*sym_0).type_0 as libc::c_uint) < ZBAR_COMPOSITE as libc::c_int as libc::c_uint
                    && (*sym_0).type_0 as libc::c_uint > ZBAR_PARTIAL as libc::c_int as libc::c_uint
                {
                    /* move to composite */
                    *symp = (*sym_0).next;
                    (*syms).nsyms -= 1;
                    (*sym_0).next = 0 as *mut zbar_symbol_t;
                    if (*sym_0).type_0 as libc::c_uint <= ZBAR_EAN5 as libc::c_int as libc::c_uint {
                        addon = sym_0
                    } else {
                        ean = sym_0
                    }
            ...
            if !ean.is_null() {
                ...
                __assert_fail(
                    b"ean\x00" as *const u8 as *const libc::c_char,
                    b"zbar/img_scanner.c\x00" as *const u8 as *const libc::c_char,
            ...
            if !addon.is_null() {
                ...
                __assert_fail(
                    b"addon\x00" as *const u8 as *const libc::c_char,
                    b"zbar/img_scanner.c\x00" as *const u8 as *const libc::c_char,
            ...
            let mut datalen: libc::c_int = (*ean)
                .datalen
                .wrapping_add((*addon).datalen)
                .wrapping_add(1 as libc::c_int as libc::c_uint)
                as libc::c_int;
            ...
            memcpy(
                (*ean_sym).data as *mut libc::c_void,
                (*ean).data as *const libc::c_void,
                (*ean).datalen as libc::c_ulong,
            );
            memcpy(
                (*ean_sym).data.offset((*ean).datalen as isize) as *mut libc::c_void,
                (*addon).data as *const libc::c_void,
                (*addon).datalen.wrapping_add(1 as libc::c_int as libc::c_uint) as libc::c_ulong,
            );
    ...
    if (*syms).nsyms != 0 && (*iscn).handler.is_some() {
        (*iscn).handler.expect("non-null function pointer")(img, (*iscn).userdata);
    }
    return (*syms).nsyms;
}
