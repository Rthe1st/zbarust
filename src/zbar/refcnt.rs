 ...
 *------------------------------------------------------------------------*/
#[no_mangle]
pub unsafe extern "C" fn _zbar_refcnt(mut cnt: *mut refcnt_t,
                                      mut delta: libc::c_int) -> libc::c_int {
    ...
    *cnt += delta;
    ...
    if rc >= 0 as libc::c_int {
        ...
        __assert_fail(b"rc >= 0\x00" as *const u8 as *const libc::c_char,
                      b"zbar/refcnt.c\x00" as *const u8 as
                          *const libc::c_char,
                      68 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 34],
                                                &[libc::c_char; 34]>(b"int _zbar_refcnt(refcnt_t *, int)\x00")).as_ptr());
    }
    return rc;
}
#[no_mangle]
...
#[no_mangle]
pub static mut _zbar_reflock: pthread_mutex_t =
...
#[no_mangle]
pub unsafe extern "C" fn _zbar_refcnt_init() {
    pthread_once(&mut initialized,
                 Some(initialize as unsafe extern "C" fn() -> ()));
}
