...
extern {
     ...
     */
    #[no_mangle]
    fn zbar_symbol_set_get_size(symbols: *const zbar_symbol_set_t) -> libc::c_int;
}
...
static mut symbolset_doc: [libc::c_char; 49] = [
    115, 121, 109, 98, 111, 108, 32, 114, 101, 115, 117, 108, 116, 32, 99, 111, 110, 116, 97, 105,
    110, 101, 114, 46, 10, 10, 99, 111, 108, 108, 101, 99, 116, 105, 111, 110, 32, 111, 102, 32,
    115, 121, 109, 98, 111, 108, 115, 46, 0,
];
unsafe extern fn symbolset_clear(mut self_0: *mut zbarSymbolSet) -> libc::c_int {
    if !(*self_0).zsyms.is_null() {
        let mut zsyms: *mut zbar_symbol_set_t = (*self_0).zsyms as *mut zbar_symbol_set_t;
        (*self_0).zsyms = 0 as *const zbar_symbol_set_t;
        zbar_symbol_set_ref(zsyms, -(1 as libc::c_int));
    }
    return 0 as libc::c_int;
}
unsafe extern fn symbolset_dealloc(mut self_0: *mut zbarSymbolSet) {
...
unsafe extern fn symbolset_iter(mut self_0: *mut zbarSymbolSet) -> *mut zbarSymbolIter {
    return zbarSymbolIter_FromSymbolSet(self_0);
}
#[no_mangle]
pub unsafe extern fn symbolset_length(mut self_0: *mut zbarSymbolSet) -> Py_ssize_t {
    if !(*self_0).zsyms.is_null() {
        return zbar_symbol_set_get_size((*self_0).zsyms) as Py_ssize_t;
    }
    return 0 as libc::c_int as Py_ssize_t;
}
static mut symbolset_as_sequence: PySequenceMethods = unsafe {
    {
        let mut init = PySequenceMethods {
            sq_length: ::std::mem::transmute::<
                Option<unsafe extern fn(_: *mut zbarSymbolSet) -> Py_ssize_t>,
                lenfunc,
            >(Some(
                symbolset_length as unsafe extern fn(_: *mut zbarSymbolSet) -> Py_ssize_t,
            )),
            sq_concat: None,
            sq_repeat: None,
            sq_item: None,
...
#[no_mangle]
pub static mut zbarSymbolSet_Type: PyTypeObject = unsafe {
    {
        let mut init = _typeobject {
            ob_refcnt: 1 as libc::c_int as Py_ssize_t,
            ob_type: 0 as *const _typeobject as *mut _typeobject,
            ob_size: 0,
            tp_name: b"zbar.SymbolSet\x00" as *const u8 as *const libc::c_char,
            tp_basicsize: ::std::mem::size_of::<zbarSymbolSet>() as libc::c_ulong as Py_ssize_t,
            tp_itemsize: 0,
            tp_dealloc: ::std::mem::transmute::<
                Option<unsafe extern fn(_: *mut zbarSymbolSet) -> ()>,
                destructor,
            >(Some(
                symbolset_dealloc as unsafe extern fn(_: *mut zbarSymbolSet) -> (),
            )),
            tp_print: None,
            tp_getattr: None,
            tp_setattr: None,
            ...
            tp_as_sequence: &symbolset_as_sequence as *const PySequenceMethods
                as *mut PySequenceMethods,
            ...
            tp_iter: ::std::mem::transmute::<
                Option<unsafe extern fn(_: *mut zbarSymbolSet) -> *mut zbarSymbolIter>,
                getiterfunc,
            >(Some(
                symbolset_iter as unsafe extern fn(_: *mut zbarSymbolSet) -> *mut zbarSymbolIter,
            )),
            tp_iternext: None,
...
#[no_mangle]
pub unsafe extern fn zbarSymbolSet_FromSymbolSet(
    mut zsyms: *const zbar_symbol_set_t,
) -> *mut zbarSymbolSet {
    let mut self_0: *mut zbarSymbolSet =
        _PyObject_New(&mut zbarSymbolSet_Type) as *mut zbarSymbolSet;
    if self_0.is_null() {
        return 0 as *mut zbarSymbolSet;
    }
    if !zsyms.is_null() {
        let mut ncsyms: *mut zbar_symbol_set_t = zsyms as *mut zbar_symbol_set_t;
